...
class QuizBot:
    ...
    async def question_scheduler(self):
        """Schedule the three daily questions"""
        post_times = [time(8, 0), time(12, 0), time(18, 0)]
        
        while True:
            now = datetime.now(GAZA_TZ)
            next_post = min(
                (datetime.combine(now.date(), t, GAZA_TZ) for t in post_times if t > now.time()),
                default=datetime.combine(now.date() + timedelta(days=1), post_times[0], GAZA_TZ)
            )
            
            sleep_time = (next_post - now).total_seconds()
            logger.info(f"Next question in {sleep_time / 60:.1f} minutes")
    ...
    async def test_cmd(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text(f"✅ Test question sent.")
        except Exception as e:
    ...
    async def run(self):
        ...
        asyncio.create_task(self.question_scheduler())
